...
namespace TrustedUninstaller.Shared
{
    ...
    public static class AmeliorationUtil
    {
        ...
        public static async Task PreDownloadDependenciesAsync(IEnumerable<string> dependencies)
        {
            foreach (var dependency in dependencies)
            {
                var url = $"https://example.com/dependencies/{dependency}.zip";
                var downloadPath = Path.Combine(Path.GetTempPath(), "AME", "Dependencies", $"{dependency}.zip");

                using (var webClient = new WebClient())
                {
                    Console.WriteLine($"Downloading {dependency} dependency, please wait...");
